...
func exportQueries(scope string) ([]exportQuery, error) {
	...
	all := []exportQuery{
		...
		{Entity: "raw_record", Query: `select id, profile_id, source_name, source_table, source_key, record_kind, payload_json, observed_at from raw_records where lower(source_table) not in ('loginkeyinfotable', 'auth_cache') order by profile_id, source_name, source_table, id`},
	}
	switch scope {
	case "all":
		return all, nil
